...
class CatalogPost(Base, TimestampMixin):
    ...
    # Медиа контент
    media_type: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)  # 'photo', 'video', 'animation'
    ...
    media_json: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)  # Массив file_id для альбомов
    
    # Автор услуги (может отличаться от создателя карточки)
    ...
    ad_frequency: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)  # Частота показа рекламы
...
class CatalogReview(Base, TimestampMixin):
    ...
    # Карточка каталога
    catalog_post_id: Mapped[int] = mapped_column(Integer, ForeignKey("catalog_posts.id", ondelete="CASCADE"), index=True)
...
class UserSession(Base, TimestampMixin):
    """Модель сессии пользователя для отслеживания просмотренных постов"""
    ...
    # Просмотренные посты (JSON массив ID)
    viewed_posts: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True, default=list)
    
    # Активность сессии
    session_active: Mapped[bool] = mapped_column(Boolean, default=True)
...
class Subscription(Base, TimestampMixin):
    """Модель подписок пользователей на категории"""
    ...
    # Тип подписки
    subscription_type: Mapped[str] = mapped_column(String(50))  # 'category'
    subscription_value: Mapped[str] = mapped_column(String(255))  # Название категории
    
    __table_args__ = (
        UniqueConstraint('user_id', 'subscription_value', name='unique_user_subscription'),
    )
    
    def __repr__(self):
        return f"<Subscription user:{self.user_id} -> {self.subscription_value}>"
